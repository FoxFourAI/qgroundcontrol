...
FlightMap {
    ...
    MapPolyline{
            ...
            Connections {
                target:                 QGroundControl.multiVehicleManager
                function onActiveVehicleChanged(activeVehicle) {
                    gpsTrajectoryPolyline.path = _activeVehicle ? _activeVehicle.autopilotPlugin.vioTrajectory.list(trajectoryPolyline._src) : []
                }
            }

            Connections {
                target:                             _activeVehicle ? _activeVehicle.autopilotPlugin.vioTrajectory : null
                function onPointAdded(coordinate) { gpsTrajectoryPolyline.addCoordinate(coordinate) }
                function onUpdateLastPoint(coordinate) { gpsTrajectoryPolyline.replaceCoordinate(gpsTrajectoryPolyline.pathLength() - 1, coordinate) }
