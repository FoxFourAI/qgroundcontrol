...
ApplicationWindow {
    ...
    flags: Qt.Window | (ScreenTools.isAndroid ? Qt.ExpandedClientAreaHint
    ...
    signal shortcutPressed(string shortcut, string result)

    //-------------------------------------------------------------------------
    //-- Global Scope Shortcuts
    ListModel {
        property var parameterSetter: QGroundControl.corePlugin.parameterSetter
        property var onboardComputersManager: globals.activeVehicle ? globals.activeVehicle.autopilotPlugin.onboardComputersManager ?? null : null
        property var activeCamera: globals.activeVehicle ? globals.activeVehicle.cameraManager.currentCameraInstance ?? null : null
        property var currentComputerId: onboardComputersManager ? onboardComputersManager.currentComputerComponent ?? null : null
        id: globalShortcuts
        ListElement {
        ...
        ListElement {
            ...
            action: function () {
                ...
                globalShortcuts.activeCamera.trackingEnabled
                        = !globalShortcuts.activeCamera.trackingEnabled
        ...
        ListElement {
            ...
            action: function () {
                ...
                let exposure = Math.floor(paramSetter.getParameter(
                                              compId, "CAM_EXPOSURE"))
        ...
        ListElement {
            ...
            action: function () {
                let compId = globalShortcuts.currentComputerId
                let paramSetter = globalShortcuts.parameterSetter
                let guideType = paramSetter.getParameter(compId,
                                                         "MISSN_GUID_TYPE")
                ...
                paramSetter.setParameter(compId, "MISSN_GUID_TYPE",
                                         newGuideType)
        ...
        ListElement {
            ...
            action: function () {
                let compId = globalShortcuts.currentComputerId
                let paramSetter = globalShortcuts.parameterSetter
                let velocity = Math.floor(paramSetter.getParameter(
                                              compId, "MISSN_TERM_VEL"))
        ...
        ListElement {
            ...
            action: function () {
                let compId = globalShortcuts.currentComputerId
                let paramSetter = globalShortcuts.parameterSetter
                let mode = Math.floor(paramSetter.getParameter(
                                          compId, "MISSN_AUTONOMY"))
                let newMode = mode + 1
                if (newMode > 2) {
                    newMode = 0
                }
    ...
    function allowViewSwitch(previousValidationErrorCount = 0, showErrorOnDisallow = true) {
        ...
        if (!allowed && showErrorOnDisallow) {
            validationErrorToast.text = qsTr(
                        "Please correct the invalid value before continuing")
            if (validationErrorToast.visible) {
                validationErrorToast.close()
            }
            validationErrorToast.open()
    ...
    function showAnalyzeTool() {
        showTool(qsTr("Analyze Tools"),
                 "qrc:/qml/QGroundControl/AnalyzeView/AnalyzeView.qml",
                 "/qmlimages/Analyze.svg")
    }

    function showVehicleConfig() {
        showTool(qsTr("Vehicle Configuration"),
                 "qrc:/qml/QGroundControl/VehicleSetup/VehicleConfigView.qml",
                 "/qmlimages/Gears.svg")
    }
    ...
    function showKnownVehicleComponentConfigPage(knownVehicleComponent) {
        showVehicleConfig()
        let vehicleComponent = globals.activeVehicle.autopilotPlugin.findKnownVehicleComponent(
    ...
    function showSettingsTool(settingsPage = "") {
        showTool(qsTr("Application Settings"),
                 "qrc:/qml/QGroundControl/Controls/AppSettings.qml",
                 "/res/QGCLogoWhite")
        if (settingsPage !== "") {
    ...
    Connections {
        ...
        function onShowMessageDialogRequested(owner, title, text, buttons, acceptFunction, closeFunction) {
            _showMessageDialogWorker(owner, title, text, buttons,
                                     acceptFunction, closeFunction)
        }
    }
    ...
    function checkForUnsavedMission() {
        if (planView._planMasterController.dirtyForSave
                || planView._planMasterController.dirtyForUpload) {
            let accepted = false
            _reentrantCloseGuard = true
            _showMessageDialogWorker(
                        mainWindow, qsTr("Unsaved Mission"), qsTr(
                            "You have a mission edit in progress which has not been saved/uploaded. If you close you will lose changes. Are you sure you want to close?"),
    ...
    function checkForPendingParameterWrites() {
        for (var index = 0; index < QGroundControl.multiVehicleManager.vehicles.count; index++) {
            if (QGroundControl.multiVehicleManager.vehicles.get(
                ...
                _reentrantCloseGuard = true
                _showMessageDialogWorker(
                            mainWindow, qsTr("Pending Parameter Updates"), qsTr(
    ...
    function checkForActiveConnections() {
        if (QGroundControl.multiVehicleManager.activeVehicle) {
            ...
            _reentrantCloseGuard = true
            _showMessageDialogWorker(
                        mainWindow, qsTr("Active Vehicle Connections"), qsTr(
    ...
    Popup {
        ...
        function calcXPosition() {
            if (indicatorItem) {
                var xCenter = indicatorItem.mapToItem(mainWindow.contentItem,
                                                      indicatorItem.width / 2,
                                                      0).x
                return Math.max(
                            _margins, Math.min(
                                xCenter - (contentItem.implicitWidth / 2),
                                mainWindow.contentItem.width - contentItem.implicitWidth
                                - _margins - (indicatorDrawer.padding * 2)
        ...
        background: Item {
            ...
            Rectangle {
                ...
                visible: indicatorDrawerLoader.item
                         && indicatorDrawerLoader.item._showExpand
                         && !indicatorDrawer._expanded
        ...
        contentItem: QGCFlickable {
            ...
            implicitWidth: Math.min(
                               mainWindow.contentItem.width - (2 * indicatorDrawer._margins)
                               - (indicatorDrawer.padding * 2),
                               indicatorDrawerLoader.width)
            implicitHeight: Math.min(
                                mainWindow.contentItem.height - ScreenTools.toolbarHeight
                                - (2 * indicatorDrawer._margins) - (indicatorDrawer.padding * 2),
                                indicatorDrawerLoader.height)
    ...
    function createAnalyzePage(source) {
        ...
        if (component.status !== Component.Ready) {
            console.warn("createAnalyzePage failed source:", source,
